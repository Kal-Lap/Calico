...
class CLKClique : public CLBase {
 ...
 public:
  CLKClique(int argc, char** argv, std::string name, int clique_size, bool max_k) :
    CLBase(argc, argv, name), clique_start_(clique_size), clique_end_(clique_size), max_k_(max_k)  {
    get_args_ += "c:l:K:";
    AddHelpLine('c', "k", "starting clique size", std::to_string(clique_size));
    AddHelpLine('l', "k", "ending clique size (default: same as -c)");
  }

  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      case 'c': clique_start_ = atoi(opt_arg); clique_end_ = clique_start_;  break;
      case 'l': clique_end_ = atoi(opt_arg);            break;
      default: CLBase::HandleArg(opt, opt_arg);
    }
  }

  int clique_size() const { return clique_start_; }
  int clique_start() const { return clique_start_; }
  int clique_end() const { return clique_end_; }
