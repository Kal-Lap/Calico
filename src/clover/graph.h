...
class CSRGraph {
 ...
 public:
  ...
  // Get contiguous iterator over all out_neighbors
  // Uses actual index pointers to account for potential offsets
  DestID_* out_neighbors_begin() const {
    return out_index_[0];
  }

  DestID_* out_neighbors_end() const {
    return out_index_[num_nodes_];
  }

  // Get contiguous iterator over all in_neighbors
  DestID_* in_neighbors_begin() const {
    static_assert(MakeInverse, "Graph inversion disabled but reading inverse");
    return in_index_[0];
  }

  DestID_* in_neighbors_end() const {
    static_assert(MakeInverse, "Graph inversion disabled but reading inverse");
    return in_index_[num_nodes_];
  }
